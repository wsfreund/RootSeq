...
inline float RootSeq::calcNorm0(const unsigned layerInit, const unsigned curLayer){
    ...
    for(unsigned curLyrRing=0; curLyrRing<ringsDist[curLayer]; ++curLyrRing){
        if (DEBUG) *debugFile<<setw(8)<<internal<<setfill('0')<<ringer_rings->at(layerInit+curLyrRing)<<"    ";
    ...
    if (norm[0]<stopEnergy){
        ...
        for(unsigned curLyrRing=1; curLyrRing<ringsDist[curLayer]; ++curLyrRing){
            if (!(norm[curLyrRing-1]<stopEnergy) && !fixed){
                ...
                if (DEBUG) *debugFile<<"Fixed = "<<fixed<<" and "<<norm[curLyrRing-1]<<" < "<<stopEnergy<<" = "<<(norm[curLyrRing-1]<stopEnergy)<<" norm["<<setw(2)<<setfill('0')<<curLyrRing<<"] = "<<setw(8)<<internal<<setfill('0')<<norm[curLyrRing - 1]<<" - "<<setw(8)<<internal<<setfill('0')<<fabs(ringer_rings->at(layerInit + curLyrRing-1))<<" = "<<norm[curLyrRing]<<std::endl;
            }
            else {
...
inline void RootSeq::applySequentialNorm(const float norm[], const unsigned layerInit, const unsigned curLayer){
    ...
    for(unsigned curLyrRing=0; curLyrRing<ringsDist[curLayer]; ++curLyrRing){
        ...
        if (DEBUG) *debugFile << "Ring number " <<setw(8)<<setfill('0')<<curLyrRing+layerInit+1<< " new value = (old value)" <<setw(8)<<setfill('0')<<internal<< ringer_rings->at(layerInit + curLyrRing) << " / (norm[" <<setw(2)<< curLyrRing << "]) " <<norm[curLyrRing];
